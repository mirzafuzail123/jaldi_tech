...
export default function CampaignAddAgentModal({ Modal, setModal, AgentList, campaignId, CampaignAgents, SavingLoader, setSavingLoader }) {

    const ModalRef = useRef()
    const { AddAgentsToCampaignFunc } = useContext(BackendContext)
    ...
    const handleAddAgent = (e) => {
        ...
        AddAgentsToCampaignFunc(campaignId, SelectedAgent).then(() => {
                            ...
                            <div className='mx-5 my-6'>
                                <ThemeProvider theme={theme}>
                                    <FormControl variant="standard" sx={{ m: 1, minWidth: 120, }} ref={ModalRef} >
                                        ...
                                        >
                                            {!CampaignAgents.some((agent) => agent.email === email) &&
                                                <MenuItem value={email}><FontAwesomeIcon icon={faUser} className=' relative top-[0.10rem]  mr-2  h-2 w-2 p-1 text-slate-700 border bg-gray-200 rounded-full' />  {username}</MenuItem>

                                            }

                                            {AgentList.map((agent, index) => {
                                                return (
                                                    !CampaignAgents.some((campaignAgent) => campaignAgent.email === agent.email) &&
                                                    <MenuItem key={index} value={agent.email}><FontAwesomeIcon icon={faUser} className=' relative top-[0.10rem]  mr-2  h-2 w-2 p-1 text-slate-700 border bg-gray-200 rounded-full' /> {agent.username}</MenuItem>
                                                )
                                            })}

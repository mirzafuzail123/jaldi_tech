...
export default function CampaignActivity() {
    ...
    const { SingleCampaignData, CampaignActivityListFunc } = useContext(BackendContext)
    const [CampaignActivityData, setCampaignActivityData] = useState([])
    const [loading, setloading] = useState(true)

    useEffect(() => {
        CampaignActivityListFunc(SingleCampaignData.id).then((data) => {
            setCampaignActivityData(data)
            setloading(false)
    ...
    return (
        SingleCampaignData && <>
            <CampaignDetailHeader name={SingleCampaignData.name} campaignId={SingleCampaignData.id} />
            {loading ? <Loader /> : <div>
                {CampaignActivityData.length === 0 ?
                    ...
                    </div> : <div className="relative max-h-[19rem]  w-[95%]  mx-auto  overflow-auto scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300 scrollbar-medium rounded-scrollbar mt-5">
